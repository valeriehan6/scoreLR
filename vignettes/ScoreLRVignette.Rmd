...
This function creates a train/test split for datasets with dependencies. The resulting data will be in the form of a dataframe with an added column for known match(KM) and known non match. You can specify the train/test split proportion `p`. Note `p` is between 0 and 1. 

### slr_results
This function is essentially a wrapper function that computes the results of various methods given a dataframe with the training/testing split already done.
...
### 1. ignore_dep
This function returns an SLR evaluation for the ignore dependence method. This method, like the name suggests, ignores that multivariate observations cannot be assumed to be independent, and implements kernel density estimation using all available scores.
...
This output will give you four dataframes, two for your train data (KM and KNM) and two for your test data(KM and KNM).
...
avgfeatures <- avg_features(KM_train, KM_test, KNM_train, KNM_test)

head(ignoredep$KNM_SLR, 5)

```

The output gives you a dataframe of SLRs. 


```{r}
results <- slr_results(data)
head(results$IgnoreDependence[[2]], 5)
head(results$StrictIndependentSet[[2]], 5)
head(results$AverageFeatures[[2]], 5)
...
```{r, eval=FALSE}
plot_slr_roc(data, method = "IndependentSet", num_runs = 200, alpha = 0.1, NUM_SETS = 10, seed = NULL)
```

The output gives you a ROC plot. 
...
* Include the final method in Classification performance of score-based likelihood ratios when data exhibit various
degrees of dependence
* Add more helper functions like allow for different definitions of optimality
* Enable people to upload their own data to the Shiny App
* Have a wrapper function